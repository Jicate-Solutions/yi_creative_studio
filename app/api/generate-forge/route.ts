...
import { createDirectorBrief } from '@/lib/agents/forge-creative-director'
import { assembleForgePrompt } from '@/lib/prompts/services/forge/forge-prompt-assembler'
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log('[FORGE] 🧪 ============ /api/generate-forge — EXPERIMENTAL PIPELINE FORK ============')
    console.log('[FORGE] Main /api/generate is UNTOUCHED. Iterate freely here.')
    ...
    // set FORGE_DETERMINISTIC_DETAILS=false to revert to AI-rendered details.
    const useDeterministicDetails =
      !useCreativeEngine && formatId === 'event_poster' && process.env.FORGE_DETERMINISTIC_DETAILS !== 'false'
      ...
      if (shouldUseXmlPrompts && provider === 'google' && formatId) {
        ...
        if (useCreativeEngine) {
          ...
          try {
            ...
            const assembled = assembleForgePrompt(directorResult.output, {
...
`
        ...
        if (provider === 'google') {
          ...
          regenerateWithEmphasis = async (missing: string[]) => {
            ...
            const r = await generateWithGemini(promptData.prompt + emphasis, promptDesignData, promptData.systemPrompt, selectedFormat, resolution, model, 0, thinkingLevel, useImageSearch)
            return r.imageBase64
          }
...
async function generateWithGemini(
  ...
  // The prompt envelope (assembleForgePrompt styleReferenceNote) tells the model this
