...
import { assembleLabPrompt } from '@/lib/prompts/services/lab/lab-prompt-assembler'
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log('[LAB] 🧪 ============ /api/generate-lab — EXPERIMENTAL PIPELINE FORK ============')
    console.log('[LAB] Main /api/generate is UNTOUCHED. Iterate freely here.')
      ...
      if (shouldUseXmlPrompts && provider === 'google' && formatId) {
        ...
        if (useLabDirector) {
          try {
            ...
            const assembled = assembleLabPrompt(directorResult.output, {
              ...
              // are no logo bars — don't append the top-40%/bottom-18% safety net.
...
async function generateWithGemini(
  ...
  // The prompt envelope (assembleLabPrompt styleReferenceNote) tells the model this
