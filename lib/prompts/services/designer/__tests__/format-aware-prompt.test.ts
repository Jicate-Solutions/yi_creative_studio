 ...
 *
 * Run with:
 *   npx tsx lib/prompts/services/designer/__tests__/format-aware-prompt.test.ts
 *
 * Exits 0 if all assertions pass, 1 otherwise.
 */

import { getFormatProfile } from '@/lib/config/format-zones'
import { assembleDesignerPrompt } from '../designer-prompt-assembler'
import { buildUserPrompt, type DirectorInput, type DirectorOutput } from '@/lib/agents/designer-creative-director'
...
// =============================================================================
console.log('\n[Designer Format Profile] assembleDesignerPrompt() safety-net + footer gate\n')

{
  ...
  const ep = assembleDesignerPrompt(noHintOutput, {
    canvasWidth: 1080,
    canvasHeight: 1440,
    reserveTopPct: 40,
    reserveBottomPct: 18,
  ...
  const ig = assembleDesignerPrompt(noHintOutput, {
    canvasWidth: 1080,
    canvasHeight: 1080,
    reserveTopPct: 30,
    reserveBottomPct: 14,
  ...
  const yt = assembleDesignerPrompt(noHintOutput, {
    canvasWidth: 1280,
    canvasHeight: 720,
    reserveTopPct: 20,
    reserveBottomPct: 14,
  ...
  const def = assembleDesignerPrompt(noHintOutput, { canvasWidth: 1080, canvasHeight: 1440 })
