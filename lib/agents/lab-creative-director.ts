 ...
 *   imported from app/api/generate-lab/route.ts. Production /api/generate
...
export async function createDirectorBrief(
  ...
  if (!apiKey) {
    console.warn('[Lab Creative Director] ANTHROPIC_API_KEY missing — returning heuristic fallback')
  ...
  console.log(`[Lab Creative Director v54.0] Calling ${DIRECTOR_MODEL} for brief: "${input.eventName.slice(0, 60)}"`)
  ...
  try {
    ...
    console.log(`[Lab Creative Director v54.0] Token usage: input=${inputTokens} output=${outputTokens} cached=${cachedTokens} cacheCreated=${cacheCreationTokens} total=${totalTokens} latency=${latencyMs}ms`)

    // Extract text from the first text content block
    const textBlock = response.content.find((c) => c.type === 'text')
    if (!textBlock || textBlock.type !== 'text') {
      console.warn('[Lab Creative Director v54.0] No text content in response, using heuristic fallback')
      return heuristicFallback(input)
    }
    ...
    if (!parsed || typeof parsed.prosePrompt !== 'string' || parsed.prosePrompt.length < 100) {
      console.warn('[Lab Creative Director v54.0] Malformed JSON output, using heuristic fallback')
      return heuristicFallback(input)
    }
    ...
    if (options?.trackUsage) {
      try {
        ...
        console.warn('[Lab Creative Director v54.0] Usage tracking failed (non-fatal):', trackErr)
      }
    }
    ...
    console.error('[Lab Creative Director v54.0] API call failed, using heuristic fallback:', err instanceof Error ? err.message : err)
    return heuristicFallback(input)
  }
}
