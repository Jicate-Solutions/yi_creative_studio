...
export function CanvasCreatePage({
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      // LAB FORK: routes Gemini/Ideogram to /api/generate-lab so prompt-pipeline
      // experiments don't touch production /create. OpenAI still uses its hardened path.
      const generateEndpoint =
        modelToUse.provider === 'openai' ? '/api/generate-openai' : '/api/generate-lab'
  ...
  const handleRegenerate = useCallback(async (options: RegenerateOptions) => {
    ...
    try {
      ...
      // LAB FORK: regenerate goes to /api/generate-lab (the experimental pipeline copy).
      const regenerateEndpoint =
        regenerateModel.provider === 'openai' ? '/api/generate-openai' : '/api/generate-lab'
