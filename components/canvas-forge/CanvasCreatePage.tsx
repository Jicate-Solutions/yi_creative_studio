...
export function CanvasCreatePage({
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      // FORGE FORK: routes Gemini/Ideogram to /api/generate-forge so prompt-pipeline
      // experiments don't touch production /create. OpenAI still uses its hardened path.
      const generateEndpoint =
        modelToUse.provider === 'openai' ? '/api/generate-openai' : '/api/generate-forge'
  ...
  const handleRegenerate = useCallback(async (options: RegenerateOptions) => {
    ...
    try {
      ...
      // FORGE FORK: regenerate goes to /api/generate-forge (the experimental pipeline copy).
      const regenerateEndpoint =
        regenerateModel.provider === 'openai' ? '/api/generate-openai' : '/api/generate-forge'
