...
import { AiUnderstoodPanel } from './AiUnderstoodPanel'
...
export function CanvasCreatePage({
  ...
  // Designer Pipeline: the "AI understood this" summary returned alongside the
  // generated image (response field `designerUnderstanding`). Shown over the preview.
  const [aiUnderstanding, setAiUnderstanding] = useState<string | null>(null)
  // Designer Pipeline v1.1: structured "AI understood this" breakdown (Hero, motifs,
  // colour, typography, text effects, style mix, rendering, will-avoid) shown over the
  // generated image. Falls back to the plain `aiUnderstanding` summary when absent.
  const [aiStructured, setAiStructured] = useState<
    import('@/lib/designer-pipeline').StructuredUnderstanding | null
  >(null)
  // Designer Pipeline: PRE-generation preview (from /api/designer-preview). Lets the
  // user confirm the interpretation before spending a generation. Deterministic + no cost.
  const [previewSummary, setPreviewSummary] = useState<string | null>(null)
  // v1.1: structured breakdown for the pre-generation preview card.
  const [previewStructured, setPreviewStructured] = useState<
    import('@/lib/designer-pipeline').StructuredUnderstanding | null
  >(null)
  const [previewLoading, setPreviewLoading] = useState(false)
  // Designer Pipeline v2: the resolved role-based style blend (for the "Style Mix" card).
  // Populated by both /api/designer-preview (pre-gen) and /api/generate-designer (post-gen).
  const [styleBlendPlan, setStyleBlendPlan] = useState<import('@/lib/designer-pipeline').StyleBlendPlan | null>(null)
  ...
  // ── Designer Pipeline pre-generation preview ──────────────────────────────
  // High-signal field values that should refresh the "AI understood this" preview.
  const _ufd = (formData.formData ?? {}) as Record<string, unknown>
  const previewEventName = String(_ufd.eventName ?? _ufd.title ?? _ufd.postTitle ?? _ufd.eventTitle ?? '')
  const previewIdea = String(_ufd.userVisualIdea ?? _ufd.creativeDirection ?? '')
  const previewBgStyle = String(_ufd.backgroundStyle ?? '')
  const previewStyleStack = JSON.stringify(Array.isArray(_ufd.styleStack) ? _ufd.styleStack : [])
  const previewLanguage = String(_ufd.language ?? 'en')
  const previewColorSig = JSON.stringify(formData.designData?.colorConfig ?? null)
  const previewFormatId = selectedFormat?.id

  // Debounced, abortable call to /api/designer-preview. Deterministic + no LLM/image
  // cost. Clears stale summary on every change and refetches when the brief changes.
  useEffect(() => {
    // Pre-generation only — once an image exists or generation runs, the post-gen
    // summary (aiUnderstanding) takes over the same card.
    if (generatedImage || isGenerating) return
    if (!currentOrganization?.id) return

    const hasEnough = previewEventName.trim().length >= 3 || previewIdea.trim().length >= 8
    if (!hasEnough) {
    ...
    setPreviewSummary(null) // clear stale immediately
    setPreviewStructured(null)

    const timer = setTimeout(async () => {
      try {
        const res = await fetch('/api/designer-preview', {
          method: 'POST',
          ...
          body: JSON.stringify({
            ...
            backgroundStyle: previewBgStyle || DEFAULT_BACKGROUND_STYLE,
            styleStack: Array.isArray(_ufd.styleStack) ? _ufd.styleStack : [],
        ...
        if (!aborted) {
          // Aborted-debounce responses come back as { aborted: true } — ignore them.
          if (data?.aborted) return
          setPreviewSummary(data.designerUnderstanding ?? data.userFacingSummary ?? null)
          setPreviewStructured(data.structuredUnderstanding ?? null)
          setStyleBlendPlan(data.styleBlendPlan ?? null)
        }
      } catch {
        if (!aborted) {
          setPreviewSummary(null) // best-effort — never block the form
          setPreviewStructured(null)
          setStyleBlendPlan(null)
        }
      } finally {
    ...
    // formData.* objects are read inside; the primitive signatures above capture the
    // meaningful changes without re-running on every unrelated keystroke.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
    previewEventName,
    previewIdea,
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      // UNIFIED PIPELINE: every model (incl. OpenAI) runs the one common designer pipeline.
      // /api/generate-designer routes to the right provider adapter internally via the render
      // router — OpenAI uses the hardened client there, so reliability is preserved.
      const generateEndpoint = '/api/generate-designer'
      ...
      // Designer Pipeline: surface how the AI interpreted the brief.
      setAiUnderstanding(result.designerUnderstanding ?? null)
      setAiStructured(result.structuredUnderstanding ?? null)
      setStyleBlendPlan(result.styleBlendPlan ?? null)
          ...
          <div className="flex-1 flex flex-col items-center justify-center overflow-hidden relative bg-[#070710]" id="preview-panel">
            ...
            {isGenerating ? (
              ...
              <div
                ...
                className="relative w-full h-full cursor-pointer flex items-center justify-center p-6 hover:bg-muted/10 transition-colors duration-200"
              >
                {(aiUnderstanding || aiStructured) && (
                  <AiUnderstoodPanel
                    summary={aiUnderstanding || ''}
    ...
    <div className="flex flex-col h-full bg-background overflow-hidden">
      ...
      {generatedImage ? (
        <div className="flex-1 min-h-0 relative bg-background">
          ...
          >
            {aiUnderstanding && <AiUnderstoodPanel summary={aiUnderstanding} styleBlendPlan={styleBlendPlan} />}
